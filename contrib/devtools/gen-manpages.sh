...
[ ! -x $BITCOIND ] && echo "$BITCOINMONEYD not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
BCMYVER=($($BITCOINMONEYCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $BITCOINMONEYD $BITCOINMONEYCLI $BITCOINMONEYTX $BITCOINMONEYQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${BCMYVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${BCMYVER[1]}//g" ${MANDIR}/${cmdname}.1
done
