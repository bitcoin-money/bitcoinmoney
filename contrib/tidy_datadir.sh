#!/bin/bash
# Copyright (c) 2013 The Bitcoin Money developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bitcoinmoney database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bitcoinmoney datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bitcoinmoney datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bitcoinmoney0.7 datadir."
    ;;
  3)
    echo "Detected Bitcoinmoney pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bitcoinmoney 0.8 datadir."
    ;;
esac
