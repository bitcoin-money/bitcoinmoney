Bitcoin Money version 0.12.0 is now available from:
...
backwards-compatible with pre-0.10 versions of Bitcoin Money or other software:
...
pre-0.12 versions of Bitcoin Money or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Bitcoin Money version 0.11 or
earlier.
...
Previous versions of Bitcoin Money had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Bitcoin Money was to raise the default minimum
relay fee.

Bitcoin Money 0.12 will have a strict maximum size on the mempool. The
...
Bitcoin Money 0.12 also introduces new default policy limits on the length and
...
Bitcoin Money 0.12 nodes. Bitcoin Money will only allow replacement of
...
Note that the wallet in Bitcoin Money 0.12 does not yet have support for
...
Bitcoin Money has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Bitcoin Money will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Bitcoin Money 0.12, when mempool limit has been reached a higher minimum
...
entirely in Bitcoin Money 0.13, and it is at this time undecided whether the
...
Bitcoin Money has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Bitcoin Money automatically creates a hidden service to listen on, without
manual configuration. Bitcoin Money will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Bitcoin Money is listening, and
...
Core to use floating fees. By default, Bitcoin Money will use floating
fees.
...
(default: `0.0002` BCMY/kB).

At all times, Bitcoin Money will cap fees at `-maxtxfee=<x>` (default:
0.10) BCMY.
Furthermore, Bitcoin Money will never create transactions paying less than
...
notes of v0.11.0](https://github.com/bitcoin-money/bitcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
Then, to tunnel a SSL connection on 28352 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 28352 -r 127.0.0.1:18332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ProxyPass http://127.0.0.1:28532/
        ProxyPassReverse http://127.0.0.1:28532/
...
- #6504 `b6fee6b` Rationalize currency unit to "BCMY" (Ross Nicoll)
